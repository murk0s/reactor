...
public class c6_CombiningPublishers extends CombiningPublishersBase {
    ...
    public void mail_box_switcher() {
        Flux<Message> myMail = mailBoxPrimary().switchOnFirst(
                (signal, messageFlux) ->
                        (signal.hasValue() && signal.get().metaData.equals("spam")) ?
                                mailBoxSecondary() :
                                messageFlux
                );
    ...
    public Mono<String> chooseSource() {
        return Mono.defer(() -> {
            ...
            return source.equals("A") ?
                    sourceA() :
                    source.equals("B") ? sourceB() : Mono.empty(); //otherwise, return empty
        });
    }
    ...
    public void cleanup() {
        ...
        Flux<Object> stream = Flux.usingWhen(
                StreamingConnection.startStreaming(),
                Function.identity(),
                monoFlux -> StreamingConnection.closeConnection(),
                (monoFlux, error) -> StreamingConnection.closeConnection(),
                monoFlux -> StreamingConnection.closeConnection()
        );
